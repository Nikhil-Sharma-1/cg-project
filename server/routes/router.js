...
router.post("/register", async(req, res) => {
    ...
    const {status, name, queued, scanning, finished, type, rule, path, pos, desc, severity} = req.body;

    if(!status || !name || !queued || !scanning || !finished) {
    ...
    try {
        ...
        if(preresult) {
            ...
            const addresult = new result({
                status, name, queued, scanning, finished, type, rule, path, pos, desc, severity
            });
